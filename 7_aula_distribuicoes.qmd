...
- Exemplo discreto: Probabilidade de Dado (Todas são = 1/6)
$P(x) = \frac{1}{x}$
...
- Uma maneira interessante de pensar no TLC é: $\bar X_n$ é aproximadamente
$N(\mu, \sigma^2 / n)$
...
- De acordo com o TCL, a média amostral $\bar X$, 
é aproximadamente normal com média $\mu$ e desvio padrão $\sigma / \sqrt{n}$
- Portanto, a probabilidade de $\bar X$ ser maior que $\mu + 2 \sigma / \sqrt{n}$
ou menor que $\mu - 2 \sigma / \sqrt{n}$ é 5%
...
paste(round(mean, 2), '(', round(conf.high, 2), '-', round(conf.low, 2), ')')
```
...
- A seguinte fórmula é uma estimativa rápida do intervalo de confiança 95%: 
